...
class GeneradorCodigoBarras:
    ...
    def imprimir_codigo(self):
        ...
        try:
            # En Windows, intentar abrir con el programa predeterminado para imprimir
            import tempfile
            with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as temp_file:
                self.codigo_generado.save(temp_file.name)
                
                if sys.platform.startswith('win'):
                    os.startfile(temp_file.name, "print")
                else:
                    messagebox.showinfo("📄 Impresión", 
                                       "Guarda el código como PNG y ábrelo con tu visor de imágenes para imprimir.")
                
        except Exception as e:
            messagebox.showerror("❌ Error de impresión", 
                                f"No se pudo imprimir el código:\n{str(e)}\n\nIntenta guardar el archivo e imprimirlo manualmente.")
